...
//			System.err.println("context: " + _directiveContext) ;
			...
			switch ( _directiveContext ) {
				...
				case "Assembler":
					directive = ASMB_parsedText(pctx.getChild(0).getChild(1)).toUpperCase().toUpperCase() ;
					worker = actx.directives().get(directive) ;
					break ;

				case "Invocation":
					directive = ASMB_parsedText(pctx.getChild(0).getChild(0)).toUpperCase().toLowerCase() ;
