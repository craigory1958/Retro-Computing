...
import xcom.retro.xa.Symbol.Reference ;
...
//		System.err.println(actx.symbols().keySet()) ;
		...
		final int monikerColWidth = actx.symbols().values().stream().map(new Function<Symbol, Integer>() {
			@Override
			public Integer apply(Symbol s) {
...
//		int lnSrcColWidth = 0 ;
		...
		for ( final Symbol symbol : Maps.sortByKey(actx.symbols()).values() )
